...
# TiWS-iForest applied to IAD with replacement of the removed trees AFTER evaluation, with data pruning (IIF-DP).
...
def detect(datasets, budget, runs):
    for dataset_info in datasets:
        ...
        for run in range(1, runs + 1):
            ...
            # Initial unsupervised training on full data set usins scikit-learn's Isolation Forest implementation
            ...
            for i in range(0, actual_budget):
                ...
                if i == 0:
                    ...
                    # Replace trees removed by TiWS-iForest in previous iteration.
                    n_replaced_trees = 100 - forest.n_estimators
                    trained_trees.append(n_replaced_trees)
                    if n_replaced_trees > 0:
                        # Data pruning: Exclude true anomalies when training new trees.
                        pruned_data_indices = [i for i in range(dataset_info.samples_count)
                                               if (i not in queried_instances or labels[i] == 0)]
                        new_forest = IsolationForest(n_estimators=n_replaced_trees, max_samples=256).fit(
                            data[pruned_data_indices])
                        forest.estimators_.extend(new_forest.estimators_)
                        forest.estimators_features_.extend(new_forest.estimators_features_)
                        forest.n_estimators = forest.n_estimators

                    # Available supervised data from previous feedback for applying TiWS-iForest

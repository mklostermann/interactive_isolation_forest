...
# IIF version 5 (simply replace 10% low precision trees (model pruning),
# but new trees are trained without anomalies (data pruning))
...
def detect(datasets, budget, runs):
    for dataset_info in datasets:
        results_dir = helper.get_results_dir(dataset_info.dataset, "iif_v5")
        ...
        for run in range(1, runs + 1):
            ...
            for i in range(0, actual_budget):
                if i == 0:
                    ...
                    new_if = IsolationForest(n_estimators=10, max_samples=256).fit(data[pruned_data_indices])

...
# IIF version 3 (version 2, but trees for replacement are selected by precision)
...
def detect(datasets, budget, runs):
    for dataset_info in datasets:
        results_dir = helper.get_results_dir(dataset_info.dataset, "iif_v3")
            ...
            for i in range(0, actual_budget):
                if i == 0:
                    ...
                    if n_trees < sk_IF.n_estimators:
                        ...
                        n_new_trees = sk_IF.n_estimators - n_trees
                        # Create new trees as replacement for dropped ones
                        new_if = IsolationForest(n_estimators=n_new_trees * 2, max_samples=256).fit(data)

                        # MKL: Some more code copied from TiWS-iForest
                        # train anomaly scores
                        _, tree_supervised = compute_tree_anomaly_scores(new_if,
                                                                         supervised_data)
                        ...
                        #  learn tree-order from supervised dataset
                        # MKL: learned_ordering sorts trees according to there performance. This will be used replace
                        # trees removed by TiWS.
                        learned_ordering = np.argsort(ap_tree_supervised)[::-1]
                        new_trees_indices = learned_ordering[0:n_new_trees]

                        sk_IF.estimators_ = list(np.array(sk_IF.estimators_)[tiws_indices])
                        sk_IF.estimators_.extend(np.array(new_if.estimators_)[new_trees_indices])
                        sk_IF.estimators_features_ = list(np.array(sk_IF.estimators_features_)[tiws_indices])
                        sk_IF.estimators_features_.extend(np.array(new_if.estimators_features_)[new_trees_indices])
